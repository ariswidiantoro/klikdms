...
<form class="form-horizontal" name="formAdd" method="post" action="" id="formAdd">
    ...
    <div class="form-group">
        <label class="col-sm-2 control-label no-padding-right" for="form-field-1">No Retur Jual</label>
        <div class="col-sm-8">
            <input type="text" required maxlength="20" autofocus name="rtj_nomer"
                   id="rtj_nomer" class="col-xs-10 col-sm-5 upper req" />
            <input type="hidden" value="" name="rtj_fkpid" id="rtj_fkpid">
            <input type="hidden" value="" name="rtj_mscid" id="rtj_mscid">
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label no-padding-right" for="form-field-1">Alasan Retur</label>
        <div class="col-sm-8">
            <textarea name="rtj_alasan_retur" class="col-xs-10 col-sm-5 upper req">
            </textarea>
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label no-padding-right" for="form-field-1">Nomer Faktur Jual</label>
        <div class="col-sm-8">
            <input type="text" required maxlength="30" name="no_faktur" id="no_faktur" class="col-xs-10 col-sm-5 req upper" />
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label no-padding-right" for="form-field-1">Nama Pelanggan</label>
        <div class="col-sm-8">
            <input type="text" readonly id="pelanggan" class="col-xs-10 col-sm-10" />
        </div>
    </div>
    <div class="form-group">
...
<script type="text/javascript">
    ...
    $(this).ready(function() {
        ...
        $('#formAdd').submit(function() {
            $.ajax({
                type: 'POST',
                url: "<?php echo site_url('transaksi_sales/saveReturJual') ?>",
                dataType: "json",
                async: false,
                data: $(this).serialize(),
                success: function(data) {
                    window.scrollTo(0, 0);
                    if(data.result){
                        $('.page-content-area').ace_ajax('loadUrl', "#transaksi_sales/returJual", true);
                    }
                    else
        ...
        $("#no_faktur").autocomplete({
            ...
            source: function(req, add) {
                $.ajax({
                    url: '<?php echo site_url('transaksi_sales/autoFakturJual'); ?>',
                    dataType: 'json',
                    type: 'POST',
                    data: {param : $("#no_faktur").val()},
                    success: function(data) {
                        add(data);
                    }
                });
            ...
            select: function(event, ui) {
                ...
                $.ajax({
                    url: '<?php echo site_url('transaksi_sales/jsonDataFaktur'); ?>',
                    dataType: 'json',
                    type: 'POST',
                    data: {param : ui.item.fkpid},
                    success: function(data) {
                        $("#rtj_fkpid").val(data['fkpid']);
                        $("#pelanggan").val(data['pel_nama']);
                        $("#rtj_mscid").val(data['mscid']);
