...
<div class="page-header">
    <h1>
        Update Model <?php echo $data['model_deskripsi'] ?>
    </h1>
</div>
<form class="form-horizontal" id="formEdit" method="post" action="<?php echo site_url('master_sales/updateModel'); ?>" name="formEdit">
    <div class="form-group">
        ...
        <div class="col-sm-8">
            <select name="model_merkid" id="model_merkid" class="form-control" style="width: 20%;">
                ...
                if (count($merk) > 0) {
                    foreach ($merk as $value) {
                        ?>
                        <option value="<?php echo $value['merkid']; ?>"<?php if($value['merkid'] == $data['model_merkid']){
                            echo 'selected';
            ...
            <input type="hidden" required="required"  value="<?php echo $data['modelid'] ?>" name="modelid" id="modelid"/>
        </div>
    </div>
    <div class="form-group">
            ...
            <select name="model_segment" id="model_segment" class="form-control" style="width: 30%;">
                ...
                if (count($segment) > 0) {
                    foreach ($segment as $value) {
                        ?>
                        <option value="<?php echo $value['segid']; ?>"<?php if($value['segid'] == $data['model_segment']){
                            echo 'selected';
                        } ?>><?php echo $value['seg_nama'] ?></option> 
                        <?php
                    }
                }
                ?>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-8">
            <input type="text" required="required" maxlength="50" style='text-transform:uppercase' 
                  value="<?php echo $data['model_deskripsi']; ?>" name="model_deskripsi" id="model_deskripsi"  class="ace col-xs-10 col-sm-8" />
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(this).ready(function() {
        $('#formEdit').submit(function() {
            $.ajax({
                type: 'POST',
                url: "<?php echo site_url('master_sales/updateModel') ?>",
                dataType: "json",
                async: false,
                data: $(this).serialize(),
