...
<form class="form-horizontal" id="formAdd" method="post" action="<?php echo site_url($etc['targetSave']); ?>" name="formAdd">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <input type="text" id="trans_noref" name="trans_noref"  maxlength="20" class="upper form-control input-xlarge" />
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <input type="text" id="trans_desc" name="trans_desc"  maxlength="50" class="upper form-control input-xxlarge" />
        </div>
    </div>
    <div id="detailtrans">
            ...
            <table id="simple-table-jasa" class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <tr  class="item-row">
                        ...
                        <td>
                            <input type="text" class="upper ace col-xs-10 col-sm-10" placeholder="DESKRIPSI" maxlength="50"
                    ...
                    <tr>
                        ...
                        <th>
                            <input type="text" readonly="readonly" style="width: 100%;text-align: right;" 
                                   value="0" name="totalDebit" id="totalTransDebit" class="col-xs-10 col-sm-10" />  
                        </th>
                        <th>
                            <input type="text" readonly="readonly" style="width: 100%;text-align: right;" 
                                   value="0" name="totalKredit" id="totalTransKredit" class="col-xs-10 col-sm-10" />  
                        </th>
                        <th class="center">
                            &nbsp;
                        </th>
...
<script type="text/javascript">
    ...
    function saveData(){
        ...
        var totalKredit = $("#totalTransKredit").val().replace(/,/g, "");
        if(!$('#formAdd').valid()){
           // e.preventDefault();
        }else{
            if(totalDebit != totalKredit ){
                alertDialog("Total transaksi DEBIT dengan KREDIT harus sama");
            }else{
    ...
    function addRow() {
                ...
                <td class="dtrans center" style="vertical-align:middle;">' + inc + '</td>\n\
                     ...
                     <td>\n\
                        <input type="text" autocomplete="off" class="upper ace col-xs-10 col-sm-10" style="width:100%;"\n\
                            placeholder = "DESKRIPSI" maxlength="50"\n\
                    ...
                    <td>\n\
                     ...
                     <td>\n\
                         <input type="text"  autocomplete="off" value= "0" onchange="$(\'#\'+this.id+\'\').val(formatCurrency(this.value));" \n\
                            onkeyup="countDebit()" \n\
                            class="number ace col-xs-10 col-sm-10" style="width:100%;text-align: right"  \n\
        ...
        function countDebit(){
            var total = 0;
            var price;
            $("input[name^=dtrans_debit]").each(function() {
                price = $(this).val().replace(/,/g, "");
                total += Number(price);
            });
            $("#totalTransDebit").val(formatCurrency(total));
        }
        
        function countKredit(){
            var total = 0;
            var price;
            $("input[name^=dtrans_kredit]").each(function() {
                price = $(this).val().replace(/,/g, "");
                total += Number(price);
            });
            $("#totalTransKredit").val(formatCurrency(total));
        }
